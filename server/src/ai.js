...
// ========== 论文写作：段落 ==========
export function genParagraph({ topic, field }) {
  const t = topic || '研究主题';
  const f = field || '该领域';
  const seed = hashStr(t + f);
  const tr = pick(TRANSITIONS, seed);
  return [
    `${tr}${t}已成为${f}研究中的重要议题。早期学者多从单一视角对其进行考察，但随着研究范式的演进，多维度、跨学科的分析框架逐渐成为主流。已有研究表明，${t}的核心机制可从理论建构、实证检验与应用转化三个层面加以理解。`,
    '',
    `具体而言，在理论建构层面，${t}涉及若干关键变量的交互关系，其作用路径并非线性，而是受到情境因素的调节。在实证检验层面，已有文献多采用面板数据或实验设计进行识别，但样本范围与度量方式的差异导致结论并不完全一致。`,
    '',
    `基于上述分析，本研究认为，对${t}的深入探讨不仅有助于完善${f}的理论体系，也能为相关实践提供更具针对性的决策参考。因此，本文将在后续章节中构建分析模型，并通过实证数据对研究假设进行系统检验。`,
  ].join('\n');
}
...
export function genFullText({ topic, field }) {
  ...
  const conclusion = [
    ...
    '',
    '——— 本论文为内置模板引擎生成的完整毕业论文（10000+字），配置真实AI模型后可获得领域更匹配的个性化内容 ———',
  ].join('\n');
