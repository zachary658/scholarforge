...
const SYSTEM_PROMPTS = {
  ...
  rewrite: '你是一位学术表达优化专家。请对用户输入的文本进行改写，通过替换同义词、调整句式、变换表达方式来优化重复表达、提升表达多样性，同时保持原意和学术性。直接输出优化后的文本，不要附加解释。',
  proposal: PROPOSAL_SYSTEM_PROMPT,
  // ===== 借鉴千笔写作新增 =====
  ai_reduce: `你是一位基于 blader/humanizer 方法论的自然表达优化专家。你的任务是改写文本，使其读起来更自然流畅，同时保留学术性与原意。

【机械化表达清单（AI-isms）—— 改写时必须逐项排查并优化】
...
- 仅输出最终改写结果，不要附加任何解释、标注或元信息。`,
  ai_reduce_versions: `你是一位基于 blader/humanizer 方法论的自然表达优化专家。请对用户提供的文本进行表达自然度优化改写，输出 3 个不同的版本，每个版本之间用单独一行 "---VERSION---" 分隔。

【要求】
1. 每个版本都要保留原文的专业术语、数据、引用与核心论点，仅优化机械化的表达方式。
...
4. 每个版本独立成段，不要附加任何解释、标注或元信息。`,
  ...
  revise: '你是一位严谨的学术论文修订专家。你会收到论文全文、审校报告与规则检查发现的问题。请据此修订论文：1) 修复报告中列出的引用、结构、数据与事实问题；2) 保持原有章节结构与学术风格，不要缩写为摘要；除非上下文明示字数不足及目标字数，否则保持原有字数规模；3) 引用编号必须与文末参考文献一一对应，不得新增无法对应的引用；4) 直接输出修订后的完整论文全文，不要输出任何解释、对比或说明。',
...
- 若某个陈述没有对应证据支撑，不要强行附证据标记，改用客观、不确定的表述，且不得给出具体数值或编造结论。`;
    ...
    case 'rewrite': {
      ...
      return `请对以下文本进行重复表达优化改写，保持原意但变换表达方式。${guide}\n\n${wrapUserContent(params.text)}${ctx}`;
    }
    case 'proposal':
    ...
    case 'ai_reduce': {
      const batched = /\[\d+\]/.test(params.text || '');
      const guide = batched ? '文本按 [1] [2] 编号分为了多个段落，请逐段改写，输出时保持对应编号（如"[1] 改写后的段落"），不要合并或删除段落。' : '';
      return `请对以下文本执行表达自然度优化改写：先依据系统提示中的 AI-isms 清单完成首轮改写，再对改写结果做二次审计——逐项排查残留的机械化表达，若有则进行第二轮改写。最终只输出优化后的文本，不要附加解释或标注：${guide}\n\n${wrapUserContent(params.text)}${ctx}`;
    }
    case 'ai_reduce_versions':
      return `请对以下文本执行表达自然度优化改写，输出 3 个不同版本，每个版本之间用单独一行 "---VERSION---" 分隔：\n\n${wrapUserContent(params.text)}${ctx}`;
    case 'defense': {
