// 版本化迁移系统测试
// 覆盖：
//   1) 启动后 schema_migrations 记录 001-004 四个版本
//   2) 迁移幂等：重复执行 runMigrations 不报错、版本不重复
//   3) 002/003/004 迁移产物真实存在（order_events 表 / projects 工作流列 / ai_tasks 重试列）
...
test('schema_migrations 记录 001-004 四个版本', () => {
  assert.deepEqual(appliedVersions(), ['001_initial', '002_order_events', '003_project_workflow', '004_task_retry']);
});
