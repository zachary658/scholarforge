...
export default function AdminPublicationOrders() {
  ...
  const approve = async (id, status) => {
    try { await api.adminApprovePublicationQuote(id, status); toast.success(status === 'approved' ? '已通过报价' : '已驳回报价'); load(); } catch (err) { toast.error(err.message); }
  };

  const updateContact = async (id, s) => {
    try { await api.adminUpdatePublicationContact(id, s); toast.success('对接状态已更新'); load(); } catch (err) { toast.error(err.message); }
  };
      ...
      <div className="card overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {items.length === 0 && (
              <tr><td colSpan={6} className="px-4 py-10 text-center text-slate-400">{loading ? '加载中…' : '暂无发表订单'}</td></tr>
            )}
          </tbody>
        </table>
      </div>
