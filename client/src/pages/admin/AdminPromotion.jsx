...
export default function AdminPromotion() {
  ...
  const addPartner = async () => {
    try { await api.adminCreatePromotionPartner({ name: partner, commission_percent: commissionPercent, settlement_hold_days: holdDays }); setPartner(''); await load(); toast.success('推广方已创建'); }
  ...
  const saveTerms = async (item) => {
    const draft = termsDraft[item.id] || { commission_percent: Number(item.commission_bps || 0) / 100, settlement_hold_days: Number(item.settlement_hold_days || 7) };
    try { await api.adminSetPromotionTerms(item.id, draft); await load(); toast.success('佣金规则已更新'); }
    catch (err) { toast.error(err.message); }
  };
  ...
  return <div className="mx-auto max-w-6xl px-6 py-8">
    ...
    <div className="mt-6 grid gap-4 md:grid-cols-2">
      <div className="card p-5"><h2 className="font-semibold">新增推广方</h2><div className="mt-3 grid gap-2 sm:grid-cols-4"><input className="input sm:col-span-2" value={partner} onChange={(e) => setPartner(e.target.value)} placeholder="推广方名称" /><input className="input" type="number" min="0" max="50" step="0.1" value={commissionPercent} onChange={(e) => setCommissionPercent(Number(e.target.value))} title="佣金比例（%）" /><input className="input" type="number" min="0" max="90" value={holdDays} onChange={(e) => setHoldDays(Number(e.target.value))} title="退款冻结期（天）" /><button className="btn-primary sm:col-span-4" onClick={addPartner}>添加（佣金 {commissionPercent}% · 冻结 {holdDays} 天）</button></div></div>
    ...
    <div className="mt-6 overflow-x-auto rounded-xl border bg-white"><div className="border-b px-4 py-3 font-semibold">推广方与佣金规则</div><table className="w-full text-sm"><thead className="bg-slate-50 text-left text-xs text-slate-500"><tr><th className="p-3">名称</th><th className="p-3">推广码/项目</th><th className="p-3">成交/佣金估算</th><th className="p-3">佣金规则</th><th className="p-3">状态与操作</th></tr></thead><tbody>{data.partners.map((p) => { const draft=termsDraft[p.id]||{commission_percent:Number(p.commission_bps||0)/100,settlement_hold_days:Number(p.settlement_hold_days||7)}; return <tr key={p.id} className="border-t"><td className="p-3">{p.name}</td><td className="p-3">{p.code_count} 个码 · {p.project_count} 个项目</td><td className="p-3">¥{Number(p.paid_amount || 0).toFixed(2)}<div className="text-xs text-slate-400">估算佣金 ¥{(Number(p.commission_cents||0)/100).toFixed(2)}</div></td><td className="p-3"><div className="flex min-w-[260px] items-center gap-2"><input className="input w-20 py-1" type="number" min="0" max="50" step="0.1" value={draft.commission_percent} onChange={(e)=>setTermsDraft({...termsDraft,[p.id]:{...draft,commission_percent:Number(e.target.value)}})}/><span>%</span><input className="input w-20 py-1" type="number" min="0" max="90" value={draft.settlement_hold_days} onChange={(e)=>setTermsDraft({...termsDraft,[p.id]:{...draft,settlement_hold_days:Number(e.target.value)}})}/><span>天</span><button className="btn-ghost text-xs" onClick={()=>saveTerms(p)}>保存</button></div></td><td className="p-3"><span className={p.is_active ? 'text-green-700' : 'text-slate-400'}>{p.is_active ? '启用' : '停用'}</span><button className="btn-ghost ml-2 text-xs" onClick={() => togglePartner(p)}>{p.is_active ? '停用' : '启用'}</button><button className="ml-2 text-xs text-red-600" onClick={() => openDelete('partner', p)}>删除</button></td></tr>; })}</tbody></table>{!data.partners.length && <div className="p-8 text-center text-sm text-slate-400">暂无推广方</div>}<p className="border-t px-4 py-3 text-xs text-slate-500">佣金为按已付款订单估算值；实际打款前请扣除退款订单，并等待对应冻结期结束。</p></div>
