...
export default function Journal() {
  ...
  const run = (orderNo) => {
    ...
    tool.run(() => api.journal({ ...form, template_id: form.template_id || undefined, orderNo: orderNo || undefined }));
  };

  const handleDownload = async () => {
    if (!docInfo?.id) return;
    try {
      await downloadDocFile(docInfo.id, form.topic || '期刊论文');
    } catch (err) {
  ...
  return (
    <div className="mx-auto flex h-full max-w-6xl flex-col px-8 py-8">
      <div className="mb-6 flex items-end justify-between">
        <div>
          <h1 className="text-xl font-bold text-ink">期刊论文撰写</h1>
          <p className="mt-1 text-sm text-slate-500">撰写符合期刊发表规范的完整学术论文，含中英文摘要</p>
        </div>
      </div>
        ...
        <div className="card flex flex-col p-6">
            ...
            {showAdvanced && (
              <div className="space-y-4 border-l-2 border-accent-100 pl-3">
                <div>
                  ...
                  <input
                    className="input"
                    placeholder="例如：实证分析、案例研究"
                    value={form.method}
                    onChange={(e) => update('method', e.target.value)}
                  />
                </div>
                <div>
                  <label className="label">目标期刊类型</label>
                  <select className="input" value={form.journalType} onChange={(e) => update('journalType', e.target.value)}>
          ...
          <button onClick={() => run()} disabled={tool.loading} className="btn-primary w-full py-3">
            {tool.loading ? (
              ...
              <><Sparkle className="h-4 w-4" /> 撰写期刊论文</>
            )}
          </button>
          {tool.error && (
        ...
        <div className="card flex flex-col overflow-hidden">
          <div className="flex items-center justify-between border-b border-slate-100 px-5 py-3">
            <span className="text-sm font-medium text-slate-600">
              {form.topic ? `${form.topic} · 期刊论文` : '生成结果'}
            </span>
            {result && (
          ...
          <div className="flex-1 overflow-y-auto p-6">
            {result ? (
              <div>
                ...
                {content ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-12 text-center">
                    ...
                    <p className="mt-4 text-sm font-medium text-ink">期刊论文已生成 Word 文档</p>
                    <p className="mt-1 text-xs text-slate-400">点击右上角「下载 Word」获取完整论文</p>
              ...
              <div className="flex h-full flex-col items-center justify-center text-center">
                ...
                <p className="mt-4 text-sm font-medium text-slate-600">填写论文信息后撰写期刊论文</p>
                <p className="mt-1 text-xs text-slate-400">符合期刊规范的完整论文，一键导出 Word</p>
              </div>
            )}
          </div>
        </div>
