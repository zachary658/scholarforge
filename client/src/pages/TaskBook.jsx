...
import {
  FileWord, Download, Refresh, Layers, Sparkle,
...
export default function TaskBook() {
  ...
  const run = (orderNo) => {
    ...
    tool.run(() => api.taskBook({ ...form, template_id: form.template_id || undefined, orderNo: orderNo || undefined }));
  };

  const handleDownload = async () => {
    if (!docInfo?.id) return;
    try {
      await downloadDocFile(docInfo.id, `${form.topic || '论文'}任务书`);
    } catch (err) {
  ...
  return (
    <div className="mx-auto flex h-full max-w-6xl flex-col px-8 py-8">
      <div className="mb-6 flex items-end justify-between">
        <div>
          <h1 className="text-xl font-bold text-ink">毕业论文任务书</h1>
          <p className="mt-1 text-sm text-slate-500">生成规范的毕业论文任务书，含进度安排与考核指标</p>
        </div>
      </div>
      ...
      <div className="grid flex-1 gap-6 lg:grid-cols-[380px_1fr]">
        ...
        <div className="card flex flex-col p-6">
          <div className="space-y-4">
            ...
            <div>
              <label className="label">学科专业 *</label>
            ...
            <div>
              <label className="label">指导教师</label>
              <input
                className="input"
          ...
          <button onClick={() => run()} disabled={tool.loading} className="btn-primary w-full py-3">
            {tool.loading ? (
              ...
              <><Sparkle className="h-4 w-4" /> 生成任务书</>
            )}
          </button>
          {tool.error && (
        ...
        <div className="card flex flex-col overflow-hidden">
          <div className="flex items-center justify-between border-b border-slate-100 px-5 py-3">
            <span className="text-sm font-medium text-slate-600">
              {form.topic ? `${form.topic} · 任务书` : '生成结果'}
            </span>
            {result && (
          ...
          <div className="flex-1 overflow-y-auto p-6">
            {result ? (
              <div>
                ...
                {content ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-12 text-center">
                    ...
                    <p className="mt-4 text-sm font-medium text-ink">任务书已生成 Word 文档</p>
                    <p className="mt-1 text-xs text-slate-400">点击右上角「下载 Word」获取完整任务书</p>
              ...
              <div className="flex h-full flex-col items-center justify-center text-center">
                ...
                <p className="mt-4 text-sm font-medium text-slate-600">填写论文信息后生成任务书</p>
                <p className="mt-1 text-xs text-slate-400">规范任务书含进度安排，一键导出 Word</p>
              </div>
            )}
          </div>
        </div>
