...
function JsonPreview({ data }) {
  ...
  if (str.length <= 80) return <span className="text-xs text-slate-600 whitespace-pre-wrap">{str}</span>;

  const [expanded, setExpanded] = useState(false);
  if (!expanded) {
    return (
      <div>
        <span className="text-xs text-slate-600 line-clamp-2 whitespace-pre-wrap">{str.slice(0, 120)}…</span>
        <button onClick={() => setExpanded(true)} className="mt-0.5 block text-accent text-xs hover:underline">展开详情</button>
      </div>
    );
  }
  return (
    <div>
      <pre className="max-h-64 overflow-auto rounded-md bg-slate-50 p-2 text-xs text-slate-700 whitespace-pre-wrap">{str}</pre>
      <button onClick={() => setExpanded(false)} className="mt-0.5 text-accent text-xs hover:underline">收起</button>
    </div>
  );
}
    ...
    <div className="mx-auto max-w-6xl px-8 py-8">
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">查看已支付毕业设计订单与需求，标记客服对接状态</p>
        </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={9} className="px-4 py-10 text-center text-sm text-slate-400">暂无毕业设计订单</td>
                </tr>
              ) : (
                items.map((it) => (
                  <tr key={it.id} className="border-b border-slate-100 text-sm last:border-0">
                    <td className="px-4 py-3 text-xs text-slate-400 font-mono">{it.id}</td>
                    ...
                    <td className="px-4 py-3">
                      <span className="text-xs text-slate-600">{it.category || '—'}</span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="max-w-[260px]">
                        <JsonPreview data={it.requirements} />
                      </div>
                    </td>
                    <td className="px-4 py-3 text-slate-700">{fmtPrice(it.quoted_price)}</td>
