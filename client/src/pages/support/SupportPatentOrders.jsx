...
export default function SupportPatentOrders() {
  ...
  const submitQuote = async (payload) => {
    setQuoteBusy(true);
    try { await api.supportQuotePatentOrder(quoteItem.id, payload); toast.success('正式报价已发送，等待用户确认'); setQuoteItem(null); load(); } catch (err) { toast.error(err.message); } finally { setQuoteBusy(false); }
  };
  ...
  return (
    <div className="px-8 py-8">
      ...
      <FormalQuoteModal open={!!quoteItem} initial={quoteItem ? { quoted_price: quoteItem.quoted_price ?? '', discipline_category: quoteItem.discipline_category || 'stem', estimated_hours: quoteItem.estimated_hours || '', quote_scope: quoteItem.quote_scope || '', quote_exclusions: quoteItem.quote_exclusions || '' } : null} busy={quoteBusy} onClose={() => setQuoteItem(null)} onSubmit={submitQuote} />
    </div>
  );
}
