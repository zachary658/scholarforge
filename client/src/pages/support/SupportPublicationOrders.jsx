...
export default function SupportPublicationOrders() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const d = await api.supportListPublicationOrders(params);
      setItems(d.items || []);
      setTotal(d.total || 0);
    } catch (err) {
  ...
  return (
    <div className="px-8 py-8">
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-ink">期刊发表对接</h1>
          <p className="mt-1 text-sm text-slate-500">共 {total} 条发表需求</p>
        </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-100 text-left text-xs text-slate-500">
              ...
              <th className="px-4 py-3">级别 / 论文</th>
              <th className="px-4 py-3">需求说明</th>
          ...
          <tbody>
            {items.map((it) => (
              <tr key={it.id} className="border-b border-slate-50">
                ...
                <td className="px-4 py-3">
                  <span className="rounded bg-slate-100 px-1.5 py-0.5 text-[10px]">{LEVEL_LABEL[it.journal_level] || it.journal_level}</span>
                  <div className="mt-1 max-w-[220px] font-medium text-ink">{it.paper_title}</div>
                  {it.field && <div className="text-xs text-slate-400">{it.field}</div>}
                </td>
                <td className="px-4 py-3 max-w-[260px] text-xs text-slate-600">
                  <div className="line-clamp-3">{(it.requirements || '').slice(0, 200) || '—'}</div>
                </td>
                <td className="px-4 py-3">
            ...
            {items.length === 0 && (
              <tr><td colSpan={6} className="px-4 py-10 text-center text-slate-400">{loading ? '加载中…' : '暂无发表需求'}</td></tr>
            )}
          </tbody>
        </table>
      </div>
