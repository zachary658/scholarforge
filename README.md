...
- **多视角蒸馏**：借鉴 STORM 架构，研究主题拆分 3-5 个检索视角，分视角检索蒸馏、跨视角去重融合
...
- **固定价格**：每个功能在管理后台「功能定价」中配置现金价格（单位：元），下单后支付
- **人工报价**：可将功能设为报价模式，用户提交需求后由客服报价、管理员审批，用户确认支付
- **支付流程**：先下单支付 → 支付成功后自动开始生成（订单状态：pending → paid → generating → completed）
...
以上均为默认值，管理后台「功能定价」可调整；每个功能受 token 成本监控保护，亏损时告警。

## 核心写作流程

1. **论文工作区**：创建论文项目，记录题目、学科、写作要求
2. **智能写作（多视角蒸馏）**：研究主题拆分 3-5 个检索视角 → 分视角多源检索同方向论文 → MapReduce 蒸馏出研究方法/创新点/结论 → 跨视角去重融合生成大纲；蒸馏产物（框架/真实文献/数据/表格）持久化到工作区，供后续章节统一消费
3. **数据套用引擎**：从检索论文的摘要与 OA 全文 PDF 提取性能指标与表格数据，用 vega-lite 重绘对比图表 / 三线表直接插入论文，图题与表格**自动标注数据来源**（"数据引自 XXX [n]"）
4. **大纲确认**：生成 3 级结构化大纲（免费），用户手动确认后方可生成全文
5. **分章节生成**：逐章生成 + 局部重写 + 章节编辑，每章注入真实文献/数据硬约束（AI 只许 `[CITE:n]` / `[CHART:x]` 引用，代码统一替换），DB 轮询实时查看进度，完成后一键合并导出
6. **数据图表**：上传 Excel/CSV 数据，vega-lite 渲染柱状/折线/饼/散点图，一键插入章节
7. **文献管理**：OpenAlex 多源检索（缓存 + 超时重试）、批量引用生成（GB/T 7714 / APA / MLA）
